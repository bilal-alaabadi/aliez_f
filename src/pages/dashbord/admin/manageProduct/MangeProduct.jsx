...
const ManageProduct = () => {
    ...
    // فرز المنتجات حسب الأقل كمية أولاً
                        ...
                        <div key={product._id} className="border rounded-lg p-3 shadow-sm w-full">
                            <div className="grid grid-cols-3 gap-2 items-center">
                                ...
                                <div className="text-right flex flex-col items-end gap-1">
                                    ...
                                    <span className={`inline-flex items-center justify-center w-8 h-8 text-white text-xs font-bold rounded-full ${
                                        Number(product.stock) < 5 ? 'bg-red-600' : 'bg-green-500'
                                    }`}>
                                        {product.stock ?? 0}
                                    </span>
                                </div>
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                ...
                                <th className="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">الكمية المتوفرة</th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {sortedProducts.map((product, index) => (
                                <tr key={product._id}>
                                    <td className="px-4 py-3 text-xs text-gray-500 text-center">{startProduct + index}</td>
                                    ...
                                    <td className="px-4 py-3 text-sm text-gray-500">{getProductPrice(product)}</td>
