...
const OrderSummary = ({ onClose }) => {
  ...
  const grandTotal = (Number(totalPrice) + Number(shippingFeeOMR)) * exchangeRate;
  const formattedTotalPrice = (Number(totalPrice) * exchangeRate).toFixed(2);
  const formattedShippingFee = (Number(shippingFeeOMR) * exchangeRate).toFixed(2);
  ...
  const getPairDiscountInfo = (item) => {
    ...
    const discountAmount = (pairs * (1 * exchangeRate)).toFixed(2);
    ...
    <div className="text-sm text-gray-800" dir="rtl">
      {products.length > 0 && (
        <div className="mb-4">
          <button
            ...
            className={`w-full flex items-center justify-between font-semibold text-gray-700 ${collapsible ? 'cursor-pointer select-none' : 'cursor-default'}`}
            aria-expanded={showItems}
          >
            <span>تفاصيل العناصر</span>
            {collapsible && (
              <span className="text-xs text-gray-500">({products.length}) {showItems ? 'إخفاء' : 'عرض'}</span>
            )}
          </button>
          ...
          {showItems && (
            <div className="mt-2 space-y-2">
              {products.map((item, i) => {
                ...
                return (
                  <div key={`${item._id || item.id || i}::${i}`} className="rounded-md border border-gray-200 p-3">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <span className="font-medium">{formattedTotalPrice} {currency}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-gray-600">الشحن</span>
          <span className="font-medium">{formattedShippingFee} {currency}</span>
        </div>
        ...
        <div className="flex items-center justify-between pt-2 border-t">
          ...
          <span className="font-extrabold text-base">{formattedGrandTotal} {currency}</span>
        </div>
      </div>

      <div className="mt-3 space-y-2">
        <Link to="/checkout" className="block">
          <button onClick={onClose} className="w-full rounded-md bg-[#42a0ec] text-white py-2.5 text-sm font-medium transition-colors">
            المتابعة للدفع
          </button>
        </Link>

        <button onClick={() => dispatch(clearCart())} className="w-full rounded-md border border-gray-300 bg-white py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
          مسح السلة
        </button>
      </div>
    </div>
